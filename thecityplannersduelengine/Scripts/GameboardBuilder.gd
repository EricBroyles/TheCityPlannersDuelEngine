...
const tri_hyp: float = sqrt(2.0 * pow(float(w) / 2.0, 2.0))
const TRI_SIZE_LVL1: Array[Vector2] = [Vector2(w,w)/2, Vector2(w,w), Vector2(0, w)] #level 1 triangle
const TRI_SIZE_LVL2: Array[Vector2] = [Vector2(0,0), Vector2(w,w), Vector2(0, w)] #level 2 right triangle
const SQU_SIZE_LVL3: Array[Vector2] = [Vector2(0,0), Vector2(0, w), Vector2(w,w), Vector2(w,0)] #level 3 square
const SQU45_SIZE_LVL3: Array[Vector2] = [Vector2(w,0), Vector2(0, w), Vector2(w,2*w), Vector2(2*w,w)] #level 3 rotated 45 square 
...
func draw_mesh() -> void:
	...
	match active_size_level:
		1: pts = PackedVector2Array(TRI_SIZE_LVL1)
		2: pts = PackedVector2Array(TRI_SIZE_LVL2)
		3: pts = PackedVector2Array(SQU_SIZE_LVL3)
	
	mesh_instance.mesh = create_polygon_mesh(pts)
